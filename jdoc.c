...
void parse_from(char* destination, char* data, char* break_string) {
  ...
  while (token != NULL) {
    ...
    if (found > 0) sprintf(result, "%s %s", result, token);
    
    // When current token is equal to break_string, set found = 1 
    if (strstr(token, break_string)) found = 1;
    // Move to next token
...
void parse_name(char* destination, char* line_data, char* break_char) {
  ...
  while (token != NULL) {
    ...
    for (int i = 0; i < NUM_PROTECTED_STRINGS; ++i ) {
      ...
      if ( (i == NUM_PROTECTED_STRINGS -1) && protected == 0 ) strcpy(destination, token);
    }

    // Move to next token
    token = strtok(NULL, DELIMS);
    // If token = break_char, end the loop
    if ( strcmp(token, break_char) == 0) break;
  }
}
...
int main(int argc, char* argv[]) {
  ...
  while (fgets(line, 1000, input) != NULL) {
    ...
    if (strstr(line, "public class") != NULL) parse_name(class_name, line, "{"), print_class_author++;
    // Store file author
    if (strstr(line, "@author") != NULL) parse_from(file_author, line, "@author"), print_class_author++;
    ...
    if (is_empty_line(line) > 0) blank_lines++; // when line is blank, increment blank_lines
